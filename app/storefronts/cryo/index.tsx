 ...
 * the class prefix (cb- -> cy-), the palette, the nav and the rail.
 *
...
export function CryoStorefront({
  page,
  storeParam = "",
  publishableKey = null,
  paypalClientId = null,
    ...
    <>
      ...
      <header className="cy-header">
        <div className="cy-wrap cy-header__in">
          ...
          <div className="cy-header__right">
            <button type="button" className="cy-cart" onClick={() => drawer?.open()} aria-label="Open cart">
              {IcoCart}
              {drawer && drawer.itemCount > 0 ? <span className="cy-cart__n">{drawer.itemCount}</span> : null}
            </button>
            <button
              type="button"
              className="cy-burger"
      ...
      <div className={`cy-menu${menu ? " is-open" : ""}`} role="dialog" aria-modal="true" aria-label="Menu">
        ...
        <div className="cy-menu__panel">
          ...
          <button type="button" className="cy-btn" onClick={() => { setMenu(false); document.getElementById("buy")?.scrollIntoView({ behavior: "smooth" }); }}>
            Buy now
          </button>
        </div>
      </div>
...
function Announce({
  ...
  return (
    ...
    >
      <div className="cy-ann__t" aria-hidden="true">
        {[0, 1].map((n) => (
          <span key={n}>
            {items.map((it) => (
...
function BuyBox({ section, page, storeParam = "", publishableKey = null, paypalClientId = null, offer = null, crowd = 0 }: { section: LoadedSection; page: LoadedProductPage; storeParam?: string; publishableKey?: string | null; paypalClientId?: string | null; offer?: { code: string; kind: string; value: number } | null; crowd?: number }) {
  ...
  return (
    <section className="cy-buy" id="buy">
      <div className="cy-wrap cy-buy__grid">
        <div className="cy-gal">
          <div className="cy-gal__main">
            <div
              className="cy-gal__track"
              ref={trackRef}
            ...
            >
              {shots.map((b, i) => (
                <div className="cy-gal__slide" key={b.id} aria-hidden={i === shot ? undefined : true}>
                  <img
            ...
            <div className="cy-gal__glow" />
            {shots.length > 1 ? (
              <div className="cy-gal__dots" aria-hidden="true">
                {shots.map((b, i) => (
          ...
          {shots.length > 1 ? (
            <div className="cy-gal__strip">
              {shots.map((b, i) => (
                <button
                  key={b.id}
                  type="button"
                  className="cy-gal__thumb"
                  aria-current={i === shot}
        ...
        <div className="cy-buy__side">
          ...
          {chosen ? (
            <div className="cy-price">
              <span className="cy-price__now">{formatMoney(chosen.priceCents, currency)}</span>
              {chosen.compareAtCents ? (
                <s className="cy-price__was">{formatMoney(chosen.compareAtCents, currency)}</s>
              ) : null}
              {off ? (
                <span className="cy-price__off">{IcoTag} Save {dollarsOff(off)}</span>
              ) : null}
            </div>
          ) : null}
          ...
          {variants.length >= 1 ? (
            ...
            <div className="cy-tiers" role="radiogroup" aria-label="Choose a bundle">
              {variants.map((x, i) => {
                ...
                return (
                  <button
                    ...
                    role="radio"
                    aria-checked={on}
                    className={`cy-tier${flag ? " cy-tier--flagged" : ""}`}
                    onClick={() => setPicked(x.id)}
                  >
                    {flag ? <span className={`cy-tier__flag cy-tier__flag--${flag[0]}`}>{flag[1]}</span> : null}
                    ...
                    {/*
                      ...
                      hidden from them.
                    */}
                    <span className="cy-tier__right">
                      <span className="cy-tier__price">{formatMoney(x.priceCents, currency)}</span>
                      {/* Only on a multi-buy. "$259.00" above "$259.00 each"
                          is the same number twice and reads as a mistake. */}
                      {qty > 1 ? (
                        <span className="cy-tier__each">
              ...
              {has(v, "bundleNote") ? (
                <div className="cy-bundle__foot">{val(v, "bundleNote")}</div>
              ) : null}
            </div>
          ) : null}
          ...
          {paypalClientId && chosen ? (
            <div className="cy-wallet cy-wallet--pp">
              <PayPalExpress
          ...
          <div className="cy-acts">
            ...
            >
              ...
              <button type="submit" className="cy-btn" disabled={!picked}>
                {val(v, "ctaLabel") || "Add to cart"}
                {chosen ? <span className="cy-btn__p">{formatMoney(chosen.priceCents, currency)}</span> : null}
              </button>
            </form>
            ...
            <form method="post" action={`/cart/add${storeParam ? storeParam + "&" : "?"}next=checkout&replace=1`}>
              <input type="hidden" name="variantId" value={picked} />
              <button type="submit" className="cy-btn cy-btn--ghost" disabled={!picked}>
                Buy now
              </button>
            </form>
          </div>
...
function PayLaterToast({
  ...
  return (
    <div className={`cy-p4t${phase === "out" ? " is-out" : ""}`} role="status">
      <img className="cy-p4t__mark" src={PAYPAL_WORDMARK} alt="PayPal" />
      <span className="cy-p4t__txt">4 &#215; {formatMoney(each, currency)}</span>
    </div>
  );
}
...
function Marquee({ brand }: { brand: CyBrand }) {
  ...
  return (
    <div className="cy-marq" aria-hidden="true">
      <div className="cy-marq__t">
        {[0, 1].map((n) => (
...
function TrustBand({ section, brand }: { section: LoadedSection; brand: CyBrand }) {
  const items = section.blocks.filter((b) => has(b.values, "title"));
  if (!items.length) return null;
  return (
    <>
      <Marquee brand={brand} />
      <section className="cy-band">
        <div className="cy-wrap cy-band__in">
          {items.map((b, i) => (
            <div className="cy-band__it" key={b.id}>
              <span className="cy-band__ico">{TRUST_ICONS[i % TRUST_ICONS.length]}</span>
              <span className="cy-band__t">{val(b.values, "title")}</span>
            </div>
          ))}
        </div>
      </section>
...
function Steps({ section }: { section: LoadedSection }) {
  ...
  if (!items.length) return null;
  return (
    <section className="cy-steps-s" id="how">
...
function ProofWall({ section }: { section: LoadedSection }) {
  ...
  return (
    <section className="cy-night" id="proof">
      {heading ? (
        <div className="cy-wrap cy-night__head">
          <h2 className="cy-h2">{heading}</h2>
          {sub ? <p>{sub}</p> : null}
        </div>
      ) : null}
      <div className={`cy-night__row cy-night__row--${shots.length}`}>
        {shots.map((shot, i) => (
          <img
  ...
  return (
    <section className="cy-vs-s">
      <div className="cy-wrap">
        ...
        <div className="cy-vs">
          <div className="cy-vs__side cy-vs__side--us">
            <h3 className="cy-vs__cap">{val(v, "usLabel") || "Us"}</h3>
            {rows.map((b) => (
              <div className="cy-vs__row" key={b.id}>
                <span className="cy-vs__what">{val(b.values, "label")}</span>
                <span className="cy-vs__val">{IcoCheck}{val(b.values, "us")}</span>
              </div>
            ))}
          </div>
          <div className="cy-vs__side cy-vs__side--them">
            <h3 className="cy-vs__cap">{val(v, "themLabel") || "Them"}</h3>
            {rows.map((b) => (
              <div className="cy-vs__row" key={b.id}>
                <span className="cy-vs__what">{val(b.values, "label")}</span>
                <span className="cy-vs__val">{IcoCross}{val(b.values, "them")}</span>
              </div>
            ))}
          </div>
        </div>
...
function WhoFor({ section }: { section: LoadedSection }) {
  ...
  if (!items.length) return null;
  return (
    <section className="cy-for-s">
      <div className="cy-wrap">
        <Head section={section} />
        <div className="cy-for">
          {items.map((b, i) => (
            <div className="cy-for__it" key={b.id} data-i={i}>
              <h3 className="cy-h3">{val(b.values, "title")}</h3>
              {has(b.values, "text") ? <p>{val(b.values, "text")}</p> : null}
            </div>
          ))}
...
function InTheBox({ section }: { section: LoadedSection }) {
  ...
  /* The old layout put a small flat-lay in one column and a list of pills in
     ...
     of it. */
  return (
    <section className="cy-carton">
      <div className="cy-carton__head">
        <h2 className="cy-carton__h">{val(v, "heading")}</h2>
        {has(v, "subheading") ? <p className="cy-carton__sub">{val(v, "subheading")}</p> : null}
      </div>
      {shot ? (
        <div className="cy-carton__stage">
          <img src={shot} alt="" loading="lazy" />
        </div>
      ) : null}
      <ol className="cy-carton__list">
        {items.map((b, i) => (
          <li key={b.id}>
            <span className="cy-carton__n">{String(i + 1).padStart(2, "0")}</span>
...
function ProofAndAnswers({ section, page }: { section: LoadedSection; page: LoadedProductPage }) {
  ...
  return (
    <section className="cy-ugc" id="ugc">
      <div className="cy-wrap">
        <div className="cy-head">
          <h2 className="cy-h2">{has(section.values, "heading") ? val(section.values, "heading") : "See it working"}</h2>
          {has(section.values, "subheading") ? (
            <p className="cy-lede">{val(section.values, "subheading")}</p>
          ) : null}
        </div>

        <div className="cy-proof">
        <div className="cy-ugc__one">
          {post ? (
            <div className={`cy-ugc__i${post.vertical ? "" : " cy-ugc__i--wide"}`}>
              <iframe
                src={post.src}
                title={post.title}
                loading="lazy"
...
const IcoPaypal = <img className="cy-pp" src={PAYPAL_MARK} alt="PayPal" />;
...
function PayLater({ page, wide = false }: { page: LoadedProductPage; wide?: boolean }) {
  ...
  return (
    <section className={`cy-pay4${wide ? " cy-pay4--under" : ""}`}>
      <div className={`${wide ? "" : "cy-wrap "}cy-pay4__in`}>
        <img className="cy-pp cy-pp--word" src={PAYPAL_WORDMARK} alt="PayPal" />
        <p>
          Pay in 4. <b>{formatMoney(each, page.store.currency)}</b> today, then three more —
          <span> interest free.</span>
        </p>
        <span className="cy-pay4__small">Subject to PayPal approval at checkout.</span>
      </div>
    </section>
  );
}
...
function SocialCard({ r, logo }: { r: LoadedProductPage["reviews"][number]; logo: string | null }) {
  ...
  if (r.channel === "notification") {
    return (
      <article className="cy-card cy-card--notif">
        {logo ? <img className="cy-card__app" src={logo} alt="" /> : null}
        <div className="cy-card__notif">
  ...
  if (r.channel === "imessage") {
    ...
    return (
      <article className="cy-card cy-card--msg">
        <span className="cy-card__from">{r.name}</span>
        <div className="cy-card__thread">
          {lines.map((line, i) => (
            <p className={i % 2 ? "cy-card__mine" : "cy-card__theirs"} key={line}>{line}</p>
          ))}
        </div>
        <span className="cy-card__when">{when}</span>
      </article>
    );
  }
  ...
  if (r.channel === "tiktok") {
    return (
      <article className="cy-card cy-card--tt">
        <span className="cy-card__av cy-card__av--tt">{r.name.replace("@", "").charAt(0).toUpperCase()}</span>
        <div className="cy-card__ttbody">
          <span className="cy-card__handle">{val(r.title ?? "")}</span>
          <p>{r.body}</p>
          <span className="cy-card__ttmeta">{when}<b>Reply</b></span>
        </div>
        {r.likes != null ? (
          <span className="cy-card__ttlike">
            {IcoHeart}
  ...
  if (r.channel === "instagram") {
    return (
      <article className="cy-card cy-card--ig">
        <header>
          <span className="cy-card__av">{r.name.trim().charAt(0).toUpperCase()}</span>
          <span className="cy-card__who">{r.name}</span>
        </header>
        {r.imageUrl ? (
          <div className="cy-card__pic">
            <Pic src={r.imageUrl} size="w640" alt="" loading="lazy" />
          </div>
        ) : null}
        <div className="cy-card__acts">{IcoHeart}{IcoComment}{IcoSend}</div>
        ...
        {r.likes != null ? <div className="cy-card__likes">{r.likes.toLocaleString("en-US")} likes</div> : null}
        <p className="cy-card__cap"><b>{r.name}</b> {r.body}</p>
        ...
        {r.replies != null ? (
          <div className="cy-card__more">View all comments</div>
        ) : null}
        ...
        {(r.title ?? "").split(String.fromCharCode(10)).filter(Boolean).map((line) => {
          ...
          return (
            <div className="cy-card__cmt" key={line}>
              <p><b>{handle}</b> {text}</p>
  ...
  return (
    <article className="cy-card cy-card--fb">
      <header>
        <span className="cy-card__av">{r.name.trim().charAt(0).toUpperCase()}</span>
        <span className="cy-card__id">
          <span className="cy-card__who">{r.name}</span>
          {r.verified ? <span className="cy-card__ok">{IcoVerified} Verified buyer</span> : null}
        </span>
      </header>
      ...
      {r.rating > 0 ? (
        <span className="cy-card__stars" aria-label={`${r.rating} out of 5`}>
          {[0, 1, 2, 3, 4].map((n) => (
            <span key={n} style={{ opacity: n < r.rating ? 1 : 0.22 }}>{IcoStar}</span>
          ))}
        </span>
      ) : null}
      <p className="cy-card__body">{r.body}</p>
      {r.imageUrl ? (
        <div className="cy-card__pic">
          <Pic src={r.imageUrl} size="w640" alt="" loading="lazy" />
        </div>
      ...
      <footer>
        {r.likes != null ? (
          <span className="cy-card__react">
            <span className="cy-card__marks">{IcoThumb}{IcoHeartFill}{IcoCareFill}</span>
            {r.likes.toLocaleString("en-US")}
          </span>
        ) : null}
        {r.replies != null ? <span className="cy-card__when">Comments</span> : null}
      </footer>
      <div className="cy-card__acts cy-card__acts--fb">
        <span>{IcoThumb2} Like</span>
        <span>{IcoComment} Comment</span>
        <span>{IcoSend} Share</span>
      </div>
      <div className="cy-card__stamp">{when}</div>
    </article>
  );
}
...
function Score({ page }: { page: LoadedProductPage }) {
  ...
  return (
    <a className="cy-score" href="#reviews">
      <span className="cy-score__stars" aria-hidden="true">
        {[0, 1, 2, 3, 4].map((n) => (
...
function UgcWall({ section }: { section: LoadedSection }) {
  ...
  return (
    <section className="cy-ugcw" id="clips">
      <div className="cy-wrap">
        <Head section={section} />
      </div>
      <div className="cy-ugcw__rail" ref={rail}>
        <div className="cy-ugcw__track">
          {[0, 1].map((pass) => (
            <div className="cy-ugcw__pass" key={pass} aria-hidden={pass === 1 ? true : undefined}>
              {clips.map((b) => (
                <figure className="cy-clip" key={`${pass}-${b.id}`}>
...
function Reviews({ section, page }: { section: LoadedSection; page: LoadedProductPage }) {
  ...
  return (
    <section className="cy-revs-s" id="reviews">
      <div className="cy-wrap">
        {has(section.values, "heading") ? <Head section={section} /> : null}
        <div className="cy-revs__head">
          {rated.length ? (
            <div className="cy-revs__line">
              <span className="cy-revs__score">{mean.toFixed(1)}</span>
        ...
        <div className="cy-rvg">
          {visible.map((r, i) => <RvCard key={r.id} r={r} eager={i < 2} />)}
        </div>

        {shown < ordered.length ? (
          <button type="button" className="cy-rvg__more" onClick={() => setShown((n) => n + 6)}>
            Read more reviews
          </button>
        ) : null}
      </div>
...
function Stars({ n }: { n: number }) {
  return (
    <span className="cy-rvstars" aria-label={`${n} out of 5`}>
      {[0, 1, 2, 3, 4].map((i) => (
...
function RvCard({ r, eager = false }: { r: LoadedProductPage["reviews"][number]; eager?: boolean }) {
  ...
  return (
    <article className="cy-rv">
      <header className="cy-rv__top">
        {r.avatarUrl ? (
          <img className="cy-rv__av cy-rv__av--pic" src={r.avatarUrl} alt="" loading="lazy" decoding="async" />
        ) : (
          <span className="cy-rv__av">{initial}</span>
        )}
        <span className="cy-rv__who">
          <b>{name}</b>
          {meta ? <span className="cy-rv__when">{meta}</span> : null}
        </span>
      </header>

      {r.rating > 0 ? <Stars n={r.rating} /> : null}
      {heading ? <p className="cy-rv__t">{heading}</p> : null}

      {thread ? (
        <div className="cy-rv__thread">
          {thread.map((line, i) => (
            <p className={i % 2 ? "cy-rv__said cy-rv__said--us" : "cy-rv__said"} key={`${i}-${line}`}>
              {line}
            </p>
          ))}
        </div>
        ...
        <>
          <p className={openBody ? "cy-rv__b is-open" : "cy-rv__b"} ref={bodyEl}>{r.body}</p>
          {clipped || openBody ? (
            <button type="button" className="cy-rv__more" onClick={() => setOpenBody((v) => !v)}>
      ...
      {r.imageUrl ? (
        /* Full width. The thumbnail it replaced loaded the 200px copy and
           ...
           nothing. */
        <div className="cy-rv__shot">
          <Pic src={r.imageUrl} size="w640" alt="" loading={eager ? "eager" : "lazy"} decoding="async" />
        </div>
      ) : null}

      {comments.length ? (
        <div className="cy-rv__cmts">
          {comments.slice(0, 2).map((c) => (
            <p className="cy-rv__cmt" key={c.who + c.said}>
              <b>{c.who}</b> {c.said}
            </p>
          ))}
        </div>
...
function Recommends({
  ...
  return (
    <section className="cy-section cy-recs" id="more">
      <div className="cy-wrap">
        <Head section={section} />
        {/* A rail, not a grid.
        ...
        <ul className="cy-recs__grid2">
          {items.map((p) => (
            <li className="cy-rec2" key={p.id}>
              <span className="cy-rec2__pic">
                ...
                {bestSaving.get(p.id) ? (
                  <span className="cy-rec2__flag">
                    {dollars(bestSaving.get(p.id)!.off)} off
                    {bestSaving.get(p.id)!.qty > 1 ? ` on ${bestSaving.get(p.id)!.qty}` : ""}
                  </span>
                ) : null}
              </span>
              <a className="cy-rec2__hit" href={`/products/${p.handle}${storeParam}`}>
                {p.title}
              </a>
              <span className="cy-rec2__price">
      ...
      <div className="cy-wrap cy-close__in">
        ...
        {buy ? (
          ...
          >
            ...
            <button type="submit" className="cy-btn">{val(v, "ctaLabel") || "Add to cart"}</button>
          </form>
        ) : null}
      </div>
    </section>
...
function HersHis({ section }: { section: LoadedSection }) {
  ...
  return (
    <section className="cy-section cy-hh" id="hers-his">
      <div className="cy-wrap">
        {has(v, "heading") ? (
          <div className="cy-head">
            <h2 className="cy-h2">{val(v, "heading")}</h2>
        ...
        <div className="cy-hh__stage">
          <img className="cy-hh__img" src={val(v, "image")} alt={val(v, "heading")} loading="lazy" />
          ...
          {notes.map((b, i) => {
            ...
            return (
              <div
                key={b.id}
                className={`cy-hh__msg${mine ? " is-mine" : ""}`}
...
function PhotoBanner({
  ...
  return (
    <section className="cy-section cy-bn">
      <div className="cy-bn__frame">
        <img className="cy-bn__img" src={val(v, "image")} alt={val(v, "heading")} />
        <div className="cy-bn__scrim" />
        <div className="cy-bn__copy">
          {has(v, "heading") ? <h2 className="cy-bn__h">{val(v, "heading")}</h2> : null}
          {has(v, "subheading") ? <p className="cy-bn__sub">{val(v, "subheading")}</p> : null}
          {pick ? (
            <form
              method="post"
              action={`/cart/add${storeParam}`}
            ...
            >
              ...
              <button type="submit" className="cy-btn cy-bn__btn">
                {val(v, "ctaLabel") || "Add to cart"}
              </button>
            </form>
          ) : null}
          {has(v, "note") ? <p className="cy-bn__note">{val(v, "note")}</p> : null}
        </div>
      </div>
    </section>
  );
...
function YardPlan({ section, page }: { section: LoadedSection; page: LoadedProductPage }) {
  ...
  return (
    <section className="cy-yard" id="plan">
      <div className="cy-wrap">
        {has(section.values, "heading") ? <Head section={section} /> : null}

        <p className="cy-yard__which">
          Showing <b>{chosen?.label}</b>. Change the build above and the plan changes with it.
        </p>

        <div className="cy-yard__grid">
          <div className="cy-yard__map">
            <svg viewBox="0 0 400 300" className="cy-yard__svg" role="img" aria-label="Overhead plan of the yard">
              ...
              {shown.map((b) => {
                ...
                return (
                  <g key={b.id} className="cy-yard__mark">
                    <circle cx={x} cy={y} r="11" fill="#F5821F" />
                    <text x={x} y={y + 4} textAnchor="middle" className="cy-yard__num">{n}</text>
                  </g>
                );
              })}
            </svg>
            <div className="cy-yard__key">
              <span>House</span><span>Driveway</span><span>Path</span><span>Sidewalk</span>
            </div>
          </div>

          <ol className="cy-yard__list">
            {shown.map((b) => (
              <li className="cy-yard__item" key={b.id}>
                <span className="cy-yard__n">{val(b.values, "n")}</span>
                <span className="cy-yard__txt">
                  <b>{val(b.values, "title")}</b>
...
function TierLadder({ section }: { section: LoadedSection }) {
  ...
  return (
    <section className="cy-ladder" id="builds">
      <div className="cy-wrap">
        {has(section.values, "heading") ? <Head section={section} /> : null}
        <div className="cy-ladder__grid">
          {tiers.map((t) => {
            const vals = lines(t.values, "values");
            return (
              <article className="cy-ladder__col" key={t.id} data-top={val(t.values, "top") ? "1" : undefined}>
                <h3 className="cy-ladder__name">{val(t.values, "name")}</h3>
                <p className="cy-ladder__price">
                  <b>{val(t.values, "price")}</b>
                  {has(t.values, "was") ? <s>{val(t.values, "was")}</s> : null}
                </p>
                <dl className="cy-ladder__rows">
                  {labels.map((label, i) => (
                    <div className="cy-ladder__row" key={label}>
                      <dt>{label}</dt>
                      <dd>{vals[i] ?? "—"}</dd>
                    </div>
                  ))}
                </dl>
                {has(t.values, "street") ? <p className="cy-ladder__street">{val(t.values, "street")}</p> : null}
              </article>
            );
          })}
        </div>
...
function InstallWeekend({ section }: { section: LoadedSection }) {
  ...
  return (
    <section className="cy-week" id="install">
      <div className="cy-wrap">
        {has(section.values, "heading") ? <Head section={section} /> : null}
        <ol className="cy-week__row">
          {steps.map((b, i) => (
            <li className="cy-week__step" key={b.id}>
              <span className="cy-week__when">{val(b.values, "when") || `Step ${i + 1}`}</span>
              <h3 className="cy-week__t">{val(b.values, "title")}</h3>
              <p className="cy-week__b">{val(b.values, "text")}</p>
            </li>
          ))}
        </ol>
        {has(section.values, "note") ? <p className="cy-week__note">{val(section.values, "note")}</p> : null}
      </div>
    </section>
  );
}
...
function WeatherPlan({ section }: { section: LoadedSection }) {
  ...
  if (!rows.length) return null;
  return (
    <section className="cy-weather" id="weather">
      <div className="cy-wrap">
        {has(section.values, "heading") ? <Head section={section} /> : null}
        <dl className="cy-weather__list">
          {rows.map((b) => (
            <div className="cy-weather__row" key={b.id}>
        ...
        {has(section.values, "promise") ? (
          <p className="cy-weather__promise">{val(section.values, "promise")}</p>
        ) : null}
      </div>
    </section>
  );
...
function StartSmaller({ section }: { section: LoadedSection }) {
  ...
  return (
    <section className="cy-two" id="either-way">
      <div className="cy-wrap">
        {has(section.values, "heading") ? <Head section={section} /> : null}
        <div className="cy-two__grid">
          {ways.map((b) => (
            <article className="cy-two__card" key={b.id}>
              <h3 className="cy-two__t">{val(b.values, "title")}</h3>
              <p className="cy-two__b">{val(b.values, "text")}</p>
            </article>
          ))}
        </div>
        {has(section.values, "note") ? <p className="cy-two__note">{val(section.values, "note")}</p> : null}
      </div>
    </section>
  );
}
...
function StickyBuy({ page, storeParam = "" }: { page: LoadedProductPage; storeParam?: string }) {
  ...
  const pic =
    ...
    null;
  return (
    <div className={`cy-sticky${on ? " is-on" : ""}`}>
      <div className="cy-wrap cy-sticky__in">
        {pic ? (
          <span className="cy-sticky__pic">
            <Pic src={pic} size="t200" alt="" loading="lazy" />
          </span>
        ) : null}
        <div className="cy-sticky__txt">
          <div className="cy-sticky__t">{page.product.title}</div>
          <div className="cy-sticky__p">
            ...
            <span className="cy-sticky__v">{buy.label}</span>
          </div>
        </div>
        <form
          className="cy-sticky__form"
          method="post"
          action={`/cart/add${storeParam}`}
          onSubmit={(e) => { if (drawer) { e.preventDefault(); drawer.add(buy.id); } }}
        >
          <input type="hidden" name="variantId" value={buy.id} />
          <button type="submit" className="cy-btn">Add to cart</button>
        </form>
      </div>
    </div>
  );
...
function Footer({ page, storeParam }: { page: LoadedProductPage; storeParam: string }) {
  ...
  return (
    <>
      ...
      <section className="cy-last">
        <div className="cy-wrap cy-last__in">
          {page.store.logoUrl ?? LOGO ? <img className="cy-last__logo" src={(page.store.logoUrl ?? LOGO) as string} alt="" /> : <span className="cy-mark cy-mark--lg">{page.store.name}</span>}
          <h2 className="cy-h2">
          ...
          {buy ? (
            ...
            >
              <input type="hidden" name="variantId" value={buy.id} />
              <button type="submit" className="cy-btn">
      ...
      <footer className="cy-footer">
        <div className="cy-wrap cy-footer__cols">
          <div className="cy-footer__brand">
            {page.store.logoUrl ? <img src={page.store.logoUrl} alt={page.store.name} /> : <b className="cy-h3">{page.store.name}</b>}
            {page.store.metaDescription ? <p>{page.store.metaDescription}</p> : null}
          </div>

          {page.nav.footer.length ? (
            <nav className="cy-footer__col" aria-label="Footer">
              <h3>Help</h3>
              <ul>
        ...
        <div className="cy-wrap cy-footer__bar">
...
function Head({ section }: { section: LoadedSection }) {
  ...
  return (
    <div className="cy-head">
      {has(v, "heading") ? <h2 className="cy-h2">{val(v, "heading")}</h2> : null}
      {has(v, "subheading") ? <p className="cy-lede">{val(v, "subheading")}</p> : null}
    </div>
  );
}
