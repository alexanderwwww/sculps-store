...
final class Shell: NSObject, NSApplicationDelegate, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate, URLSessionWebSocketDelegate {
  ...
  func applicationDidFinishLaunching(_ note: Notification) {
    ...
    if workerPort == 0 {
      fail("Research was started without a worker port. Open Research again from the Dock.")
      return
    }

    fetchAgent(deadline: Date().addingTimeInterval(20)) { source in
      guard let source = source else {
        fail("Research could not reach its worker on port \(workerPort). Quit Research and open it again.")
        return
      }
  ...
  </style><div class="b"><div class="d"></div><div>Research</div>
...
let appMenu = NSMenu()
appMenu.addItem(withTitle: "Hide Research", action: #selector(NSApplication.hide(_:)), keyEquivalent: "w")
appMenu.addItem(withTitle: "Hide", action: #selector(NSApplication.hide(_:)), keyEquivalent: "h")
appMenu.addItem(NSMenuItem.separator())
appMenu.addItem(withTitle: "Quit Research", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
