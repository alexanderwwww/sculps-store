...
async function stageSearch() {
  ...
  if (!next) { SRC.stage = "shortlist"; await saveSourcing(); return; }
...
async function sendApprovedDrafts() {
  for (const it of SRC.outbox.approved()) {
    ...
    const out = await sendApproved(SRC.outbox, it.id, async (payload) => {
      ...
      const typed = await phone.type(payload.text, { into: "message", who: "desk" });
...
export async function main() {
  ...
  setInterval(() => { maybeSource().catch((e) => say("desk", `the desk stopped: ${e?.message ?? e}`)); }, 7000).unref();
