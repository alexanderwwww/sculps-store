...
rm -rf "$RES"; mkdir -p "$RES/ui"
cp "$HERE"/worker/*.mjs "$HERE"/worker/package.json "$RES/"
cp "$HERE"/worker/ui/index.html "$RES/ui/"
...
[ -f "$C/Resources/worker/ui/index.html" ] || { echo "ui missing"; exit 1; }
